...
I got my data from a kaggle page by Berkeley Earth titled Climate Change: Earth Surface Temperature data. I chose to use the country specific dataset, as then I could easily merge it with the country data set from homework 7. From there I calculated the average temp per region, then the average temp per year. I tried having each year plotted for a tick on the x axis, but it became overcrowded quite quickly. I ran into the issue the Altair wouldn't let me plot with such a big dataset, so I cut some of it off, then dropped any year that didn't end in a 5 or a 0. Overall, I wouldn't consider this my masterpiece, as I am not happy with the final product. It did however take a long time to create, and as of now I am content with the time that I spent on it.
